...
export function SearchResults({searchString}) {
    ...
    useEffect(() => {
     ...
     if (Array.isArray(res) && res.length > 0) {
        return <div className="wrapper-res" style={{display: searchString != "" ? 'flex' : 'none' }}>
            ...
            <div className="page-control">
                    ...
                    <button id="next-page" style={{display: res.length < 20 ? 'none' : 'flex' }} onClick={(e) => handlePages("next")}>Next →</button>
                </div>
        </div>
     } else {
         return <div className="wrapper-res" style={{display: searchString ? 'flex' : 'none' }}>No content available</div>
     }
}